...
# Resolve audio file path relative to this script
...
def find_audio_player():
    ...
    # Check for ffplay (Linux, cross-platform via ffmpeg)
...
def play_sound():
    ...
    try:
        if player == "afplay":
            # macOS afplay: simple, just play the file
            subprocess.run([player, str(SOUND_FILE)], check=True, capture_output=True)
        elif player == "ffplay":
            # ffplay: hide window, auto-exit, quiet mode
            subprocess.run(
