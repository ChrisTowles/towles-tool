...
pub trait WatcherContext {
    ...
    /// Resolve an agent's OS pid to the tmux session whose pane owns it: the
    /// tmux server walks the pid's ancestry to a pane pid. Hosts without pane
    /// knowledge (the desktop app) keep the default `None`, falling back to
    /// [`WatcherContext::resolve_session`] on the cwd.
