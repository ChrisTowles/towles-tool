...
export const FILE_NESTING_PATTERNS: Readonly<Record<string, string>> = {
  ...
  "*.tsx":
    "$(capture).ts, $(capture).*.ts, $(capture).*.tsx, $(capture)_*.ts, $(capture)_*.tsx, $(capture).css, $(capture).module.css, $(capture).less, $(capture).module.less, $(capture).module.less.d.ts, $(capture).scss, $(capture).module.scss, $(capture).module.scss.d.ts, $(capture).css.ts",
  ...
  "AGENTS.md":
    ".clinerules, .cursorrules, .replit.md, .windsurfrules, AGENT.md, CLAUDE.local.md, CLAUDE.md, GEMINI.md",
  ...
  "build.gradle.kts":
    "settings.gradle.kts, gradlew, gradlew.bat, gradle.properties, gradle.lockfile",
  "Cargo.toml":
  ...
  Dockerfile:
    "*.dockerfile, .devcontainer.*, .dockerignore, Dockerfile*, captain-definition, compose.*, docker-compose.*, dockerfile*",
...
export function nestFiles(names: readonly string[]): Map<string, string[]> {
  ...
  const claims = new Map<string, { parent: string; score: number }>();

  for (const [parentGlob, childList] of Object.entries(FILE_NESTING_PATTERNS)) {
    const parentIsLiteral = !parentGlob.includes("*");
    const parentRe = globToRe(parentGlob, true);
    const parents: ParentMatch[] = [];
    for (const name of names) {
      const m = parentRe.exec(name);
      if (m) parents.push({ name, capture: m[1] ?? "" });
    }
    if (parents.length === 0) continue;

    for (const childGlob of childList.split(",")) {
      const trimmed = childGlob.trim();
      if (!trimmed) continue;
      for (const parent of parents) {
        const resolved = trimmed.replaceAll("$(capture)", parent.capture);
        const childIsLiteral = !resolved.includes("*");
        const score = claimScore(parentIsLiteral, childIsLiteral);
        // The common case is a literal child name — a set hit, no scan.
        const matches = childIsLiteral
          ? present.has(resolved)
            ? [resolved]
            : []
          : names.filter((n) => globToRe(resolved, false).test(n));
        for (const child of matches) {
          if (child === parent.name) continue;
          const prev = claims.get(child);
          if (
            prev &&
            (prev.score > score ||
              (prev.score === score && prev.parent.length >= parent.name.length))
          )
            continue;
          claims.set(child, { parent: parent.name, score });
        }
      }
    }
  }

  // One level only: resolve every chain to its top, then drop the claim on any
  // file that is itself a parent.
  const parented = new Set([...claims.values()].map((c) => c.parent));
  const nested = new Map<string, string[]>();
  for (const [child, claim] of claims) {
    if (parented.has(child)) continue;
    const list = nested.get(claim.parent);
    if (list) list.push(child);
    else nested.set(claim.parent, [child]);
  }
  for (const list of nested.values()) list.sort((a, b) => a.localeCompare(b));
  return nested;
}
